...
public class SharedPreferencesUserUtil {
	...
	private SharedPreferencesUserUtil(Context context) {
		settings = context.getSharedPreferences("XiLife_user_data", Context.MODE_PRIVATE);
	}

	public static SharedPreferencesUserUtil getInstance(Context context) {
		if (instance == null) {
			synchronized (SharedPreferencesUserUtil.class) {
				instance = new SharedPreferencesUserUtil(
	...
	public HashMap<String, String> getHashMapByKey(String key) {
		...
		synchronized (settings) {
			String mapStr = settings.getString(key+user, "{}");
