...
public class FinalDb {
	...
	public void saveForBatch(List<?> entityList){
		if(null == entityList)
			throw new NullPointerException("批量插入的数据不能为空");
		checkTableExist(entityList.get(0).getClass());
		db.beginTransaction();
		try {
			for(int i = 0;i < entityList.size();i++){
				exeSqlInfo(SqlBuilder.buildInsertSql(entityList.get(i)));
			}
			db.setTransactionSuccessful();
		} catch (Exception e) {
			Logger.logException(e , "批量插入有误");
		}finally{
			db.endTransaction();
		}
	}
	...
	public void updateForBatch(List<?> entityList){
		if(null == entityList)
			throw new NullPointerException("批量插入的数据不能为空");
		checkTableExist(entityList.get(0).getClass());
		db.beginTransaction();
		try {
			for(int i = 0;i < entityList.size();i++){
				exeSqlInfo(SqlBuilder.getUpdateSqlAsSqlInfo(entityList.get(i)));
			}
			db.setTransactionSuccessful();
		} catch (Exception e) {
			Logger.logException(e , "批量更新有误");
		}finally{
			db.endTransaction();
		}
	}
	 ...
	 *            排序的字段
	 */
	public <T> List<T> findAll(Class<T> clazz, String orderBy , String order) {
		checkTableExist(clazz);
		return findAllBySql(clazz, SqlBuilder.getSelectSQL(clazz)
